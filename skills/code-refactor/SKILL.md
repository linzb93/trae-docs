...
```bash
node .trae/skills/code-refactor/scripts/index.js --type magic-numbers
```
...
```bash
# 扫描 src 目录，自动分析最大的文件
node .trae/skills/code-refactor/scripts/index.js --type vue-split --path src

# 分析指定文件
node .trae/skills/code-refactor/scripts/index.js --type vue-split --path src/views/Home.vue
```
...
```bash
node .trae/skills/code-refactor/scripts/index.js
```
